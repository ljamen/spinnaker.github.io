...
Create a compute instance on Oracle Cloud Infrastructure (OCI) to host Spinnaker by following [this tutorial](https://www.oracle.com/webfolder/technetwork/tutorials/obe/cloud/ocis/creating_vm/bmc_vm_tutorial.html).  Spinnaker currently supports Ubuntu 14.04 only.

### Install and configure Spinnaker

SSH onto the compute instance you previously created.
...
Set up  Oracle Object Storage as your Spinnaker storage by following these [Adding Oracle Object Storage to Spinnaker](https://www.spinnaker.io/setup/install/storage/oracle/) steps.

Set up your Oracle Cloud provider account by following these [Adding Oracle Cloud Provider Account to Spinnaker](https://www.spinnaker.io/setup/install/providers/oracle/) steps.

Set up your Oracle Bakery by following these [Configuring Oracle Cloud Infrastructure Bakery](https://www.spinnaker.io/setup/bakery/oracle/) steps.
...
The purpose of this pipeline is to generate a image from a package, and then deploy the image on server groups in the test cluster. We want this pipeline to be kicked off every time the continuous integration job completes.
...
Assuming the package is built in CI (Jenkins/Travis/Wercker), we want this pipeline to kick off every time these job completes. In the Automated Triggers section of the pipelines form, add Jenkins/Travis/Wercker Trigger for your job.

In this tutorial, we will skip the trigger and use tomcat8 package.

#### Set up Bake stage

The purpose of our “Bake” stage is to create a image with the package that was built by the CI job that triggered this pipeline.  It is tomcat8 in this tutorial.
...
* In the Basic Settings section, select “codelabVCN” in the *VCN* drop down, “test” in the *Stack* field.
...
* In the Load Balancers section, select the load balancer we created earlier.
...
We create a load balancer for the prod cluster. Navigate to the LOAD BALANCERS tab and click the *Create Load Balancer* button.  Everything is the same as the codelab-test-lb load balancer created before.  The only exception is to use “prod” instead of “test” for *Stack*.
...
* In the *Server group* section, click *Add server group*.
* In the dialog that appears, click *Continue without a template*.
* In the Basic Settings section, everything is the same as the “codelab-test” server group created before.  The only exception is to use “prod” instead of “test” for *Stack*.
* In the Load Balancers section, add the “codelab-prod-lb” previously created.
* Click the *Add* button.
